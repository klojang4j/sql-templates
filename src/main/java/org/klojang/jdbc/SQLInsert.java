...
public final class SQLInsert extends SQLStatement<SQLInsert> {
  ...
  public <U> void insertAll(Collection<U> beans) {
    ...
    beans.forEach(bean -> bind(bean).execute());
  }
   ...
   * @param beans the beans to save
   */
  public <U> void insertAllAndSetIDs(String idProperty, Collection<U> beans) {
    Check.that(bindings).is(empty(), illegalState(DIRTY_INSTANCE));
    try {
      for (U bean : beans) {
   ...
   */
  public <U> long[] insertAllAndGetIDs(String idProperty, Collection<U> beans) {
    Check.that(bindings).is(empty(), illegalState(DIRTY_INSTANCE));
