...
public final class SQLInsert extends SQLStatement<SQLInsert> {
   ...
   * method returns the auto-generated key, else it returns -1.
   *
   ...
   * must not contain any bound values yet. This will be the case if the instance has just
   * been created, or you have just executed the INSERT statement (which resets the
   * instance), or you have explicitly called {@link #reset()}.
   *
   ...
   * the database. This method combines the binding and execution phase. Therefore it must
   * be called on a "fresh" instance. That is, it must not contain any bound values yet.
   * This will be the case if the instance has just been created, or if you just executed
   * the INSERT statement (which resets the instance), or if you just called
   * {@link #reset()}.
   *
   ...
   * Therefore it must be called on a "fresh" instance. That is, it must not contain any
   * bound values, beans or maps yet &#8212; which would be the case if the instance has
   * just been created, or you have just executed the INSERT statement (which resets the
   * instance), or you have explicitly called {@link #reset()}.
   *
