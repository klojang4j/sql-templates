...
public final class SQLInsert extends SQLStatement<SQLInsert> {
   ...
   *
   * <p>For large batches consider using the {@link SQLBatchInsert} class as it is likely
  ...
  /**
   * Saves the specified JavaBeans to the database and returns the auto-generated keys.
   * This method combines the binding and execution phase. Therefore it must be called on
   * a "fresh" instance. That is, it must not contain any bound values, beans or maps yet
   * &#8212; which would be the case if the instance has just been created, or you have
   * just executed the INSERT statement (which resets the instance), or you have
