 ...
 * {@link BatchInsertBuilder}. {@code SQLBatchInsert} implements {@link AutoCloseable} in
 * order to stay aligned with {@link SQLStatement} and its subclasses, and more
 * specifically so you can easily switch between {@link SQLInsert} and
 * {@code SQLBatchInsert} within your code. However, its {@code close()} method currently
 * is a no-op. It is not necessary to use a try-with-resource block with
 * {@code SQLBatchInsert}.
 *
...
public final class SQLBatchInsert<T> {
  ...
  SQLBatchInsert(BatchInsertConfig<T> cfg) {
    this.cfg = cfg; this.sqlBase = getSqlBase(cfg);
  }
  ...
  public void insertBatch(List<T> beans) {
    Check.notNull(beans); if (!beans.isEmpty()) {
  ...
  public void insertBatchAndSetIDs(String idProperty, List<T> beans) {
    Check.notNull(beans); Class<T> clazz = cfg.reader().getBeanClass();
  ...
  private void insertBatch(List<T> beans, int chunkSize) {
    for (int i = 0; i < beans.size(); i += chunkSize) {
      int j = Math.min(beans.size(), i + chunkSize); try {
  ...
  private void insertChunk(List<T> beans) throws Throwable {
    StringBuilder sql = new StringBuilder(guessSize(beans)); sql.append(sqlBase);
    try (Statement stmt = cfg.connection().createStatement()) {
      addRows(sql, stmt, beans); stmt.executeUpdate(sql.toString(), NO_GENERATED_KEYS);
    } commit();
  }

  private long[] insertChunkAndGetIDs(List<T> beans) throws Throwable {
    long[] keys; StringBuilder sql = new StringBuilder(guessSize(beans));
    sql.append(sqlBase); try (Statement stmt = cfg.connection().createStatement()) {
  ...
  private void addRow(StringBuilder sql, Statement stmt, T bean) {
    BatchInsertConfig<T> cfg = this.cfg; Quoter quoter = new Quoter(stmt);
    String[] props = cfg.reader().getReadableProperties().toArray(String[]::new);
    List<Object> values = cfg.reader().readAllProperties(bean); sql.append('(');
