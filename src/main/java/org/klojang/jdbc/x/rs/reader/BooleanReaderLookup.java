...
public final class BooleanReaderLookup extends ColumnReaderLookup<Boolean> {
  ...
  public BooleanReaderLookup() {
    addMultiple(new ColumnReader<>(GET_BOOLEAN), BOOLEAN, BIT);
    ...
    addMultiple(new ColumnReader<>(GET_DOUBLE, Bool::from), FLOAT, DOUBLE);
    put(BIGINT, new ColumnReader<>(GET_LONG, Bool::from));
    put(REAL, new ColumnReader<>(GET_FLOAT, Bool::from));
    addMultiple(new ColumnReader<>(GET_BIG_DECIMAL, Bool::from), NUMERIC, DECIMAL);
    addMultiple(new ColumnReader<>(GET_STRING, Bool::from), CHAR, VARCHAR);
  }
