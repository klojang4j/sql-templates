...
public final class EnumReaders extends ResultSetReaderLookup<Enum<?>> {

  public EnumReaders() {
    add(BIGINT, new ResultSetReader<>(GET_LONG, new NumberAdapter<>()));
    add(INTEGER, new ResultSetReader<>(GET_INT, new NumberAdapter<>()));
    add(SMALLINT, new ResultSetReader<>(GET_SHORT, new NumberAdapter<>()));
    add(TINYINT, new ResultSetReader<>(GET_BYTE, new NumberAdapter<>()));
