...
public sealed interface ResultSetBeanifier<T> extends Iterable<T>
     ...
     * an empty {@code Optional} is returned. You can keep calling {@code beanify()} to
    ...
    /**
     * Converts at most {@code limit} rows from the {@code ResultSet} into JavaBeans. If
     * the {@code ResultSet} is empty, an empty {@code List} is returned.
     *
     * @param limit the maximum number of rows to extract and convert
     * @return a {@code List} of JavaBeans or an empty {@code List} if the
